...
export const createPersonalizationSchema = z.object({
  product_id: z.string().trim().min(1, { message: "Product ID is required" }),

  theme_id: z.string().trim().min(1, { message: "Theme ID is required" }),

  forWhom: z.string().trim().min(1, { message: "For whom is required" }),

  order_id: z.string().trim().min(1, { message: "Order ID is required" }),
});
...
export const selectPersonalizationJokesSchema = z.object({
  ...
  joke_ids: z
    .array(z.string().trim().min(1, { message: "Joke ID is required" }))
    .min(5, {
